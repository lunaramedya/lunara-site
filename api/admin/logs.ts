...
export default async function handler(req: RequestLike, res: ResponseLike) {
  try {
    ...
    const rows = await runQuery(process.env, 'SELECT * FROM event_logs ORDER BY created_at DESC LIMIT $1', [limit]);
    res.status(200).json({ ok: true, items: rows });
  } catch (error) {
    res.status(500).json({
      ok: false,
      message: error instanceof Error ? error.message : 'Log verileri alınırken hata oluştu.',
    });
  }
}
